...
const ingRes = await fetch('data/ingredients.json');
allIngredients = await ingRes.json();


const synRes = await fetch('data/synonyms.json');
...
const suggestions = allIngredients.filter(i=>i.toLowerCase().includes(q));
